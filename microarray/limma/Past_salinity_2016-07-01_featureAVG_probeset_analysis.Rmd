...
https://github.com/ljcohen/Porites_astreoites_larvae_salinity/blob/master/microarray/limma/Past_salinity_2016-07-01_featureAVG_probeset_analysis.Rmd
...
library(ggplot2)
library(lattice)
setwd("~/Documents/HBOI/Past_salinity/Porites_astreoites_larvae_salinity/microarray/limma")
...
setlist <- list(sal25ppt_v_sal30ppt_padj0.25=as.vector(tt_25ppt_v_30ppt_sig$Name),sal25ppt_v_sal35ppt_padj0.25=as.vector(tt_25ppt_v_35ppt_sig$Name),sal25ppt_v_sal30ppt_padj0.25=as.vector(tt_25ppt_v_30ppt_sig$Name))
...
colorfunc <- colorRampPalette(c("red", "green"))
clusterCols <- rainbow(length(unique(mycl)))
clusterCols <- viridis(45, option = "D")
clusterCols <- colorfunc(2)
...
heatmap.2(d, main="25ppt vs. 35 ppt (padj<0.05,log2FC+-1)", 
          ...
          adjCol = c(NA,0),offsetCol=5, 
          Colv=NA, dendrogram="row", 
          scale="row", col=myheatcol, 
          density.info="none", 
          trace="none", RowSideColors= myClusterSideBar,edgePar=list(lwd=2))
#dev.off()
dev.print(jpeg, file = "~/Desktop/heatmap_aRt.jpeg", width = 15000, height = 12000, units = "px", res = 300)
dev.off()
```
